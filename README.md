...
### What Use Might This Project Have in High School Curriculum?
This project is an excellent tool for high school science curriculum, offering students hands-on experience with STEM concepts while encouraging critical thinking and hypothesis testing. The sensor’s flexibility, made possible through its programmable library, allows students to explore a variety of experiments and customize its functionality to suit their needs. For instance, the library includes functions that enable users to adjust calibration curves or test alternative parameters, such as using different color indicators for measurements.
...
it is important to think of your use case for the sensor and if you will need sturdier materials or if you don't plan to deploy it on water at all instead from a dock.
|ITEM|QUANTITY|PRICE|
|---|---|---|
...
This also does not include adhesive, I would recommend getting yourself some kind of strong sealant for creating the seal around the bottom of the cuvette and the valve however you could in theory use hot glue if you're good enough with it.
### Perfboard or Breadboard?
Perfboards are used when you are creating electrical circuits and would like a stable connection that doesn't require having jumper cables. This
...
break. If you are going with perfboards I recommend a breadboard layout as there are the easiest to use as they have power buses(See Vocabulary)
built in. If you are going to use perfboards I would recommend using a bread board to make sure that you have a good understanding of the 
...
Breadboards on the other hand are less permanent but can have issues with pins not touching metal properly and are often seen as temporary solutions.
...
### Wires or Jumpers?
Choosing between buying wires or jumpers is very difficult. If possible one should buy both as it allows for modularity when creating the ciruits however
...
are properly up to par with solder before permanently setting things into place. I recommend starting with watching this video:
...
or an empty signature. Do that and see if those files are now in the pico section, if they are you succeeded in downloading those files onto your pico. Try also
creating a file on the pico, right click under the raspberry pi pico section and then click create file. Name said file test.py, this should open the file on 
...
Then click crtl s and save the file. You should see the file appear on the bottom left side of your screen as test.py. You can now run your file using the green check mark in
...
If you have a strip that us longer then you can use wire cutters to cut it down to size. Try and solder in a diagonal pattern, soldering one corner then another.
Remember that you should also be soldering the UNDERSIDE of the board and not the top. This along with a stabilization method(Tape the item into place in the front side) 
...
The following picture is an example of a pico slotted into header pins. (Note your female pins should be soldered onto the board not the pico)
...
to the blue side and the power to the red side. Remember one bus is 3 volts and the other is 12 volts so you should have a 3 volt battery pack and a 12 volt battery
pack. 

![PowerBus](Photos/CircledBus.jpg)

If you have a multi meter you can test your work by powering the battery and putting one side to positive and another to ground on the board and then switch to voltage.
...
correct voltage read on the multi meter. If the voltage seems slightly off then check to see if your AA batteries are fully charged. Next we are going to solder male headers 
...
Now we will be soldering one end of the wire directly next to the ground of the pico that is highlighted above and the other end to the ground bus of the 3V battery pack. 
...
instead you are wiring to empty rows on the board. You should leave one empty slot between the cable and the middle gap as that is where the resistors are going. I recommend wiring
...
Once those headers are soldered you are all done with the perf board! Congratulations!
...
## Printing Your Parts
Now that you have got your soldering done its time to go about 3D printing the required parts in the CAD folder of the repo. All designing is already done for you so you can take
those files to a 3D printer, enter them, and begin the printing while we connect all of the wiring together. If you don't have a 3D printer check your local library and if they don't
...
Poke three holes in the boogie board on each side near the pool noodles. Through each hole run zip ties through and around the pool noodles to strap them into place. Finally attach the
final two halves of pool noodle under the boogie board and strap them to the boogie board using the same zip tie method.

![Ziptie](Photos/ZipSideView.jpg)

Now we are going to drill holes in the side of our box to run cables out. They should just be big enough to fit a cable gland through, once it is through you should tighten the nut on the
inside and using the rubber seal. Attached will be a video link that shows how to do so [here](https://www.facebook.com/waterproofcablegland/videos/how-to-install-the-cable-gland-to-the-junction-box/700374587211266/)
...
board and run a zip tie through it and the ruler and lock it straight down. You will then need to secure you DV pump to the bottom of the ruler as seen here:

![HighlightingPump](Photos/Highlighting%20Pump.jpg)

You will also have to run tubing to the pump so it is important to make sure that it isn't flailing around. Zip ties can do the job just remember not to cut off any flow of the pump as it is 
very delicately measured in terms of timing and anything could change said timing. Once the tubing to the pump runs all the way up you want to make sure that you cut it to size. That is where 
the printed parts come in. You should now have your parts hopefully and can install them. The first thing to be installed will be the cuvette holder. This can be installed by drilling a hole 
...
the bulk shipment as they tend to be somewhat flimsy. Once done with that you should then use your adhesive of choice to secure the tubing to the bottom of the cuvettes. It should only be about 
...
That tubing can be applied to one end of the valve and then run to the plastic waste container which then have to have a subsequent hole cut through it and adhesive will have to join the tubing
with said container. Now your cuvettes should be dry, make sure to test that they aren't leaking as if they are it can cause damage to the electronics and will make it almost impossible to get a
...
put the peristaltic pump into the cuvette cover. It is a very snug fit so don't be afraid to really push it in there. If it is to tight to push in you can use a file and get rid of some of the 
material. On the other side take out the syringe plunger, remove the tip from the syringe and place the whole syringe into the side of the cuvette cover that doesn't have the pump in it. You 
can then cut the syringe down to size and make sure that it sits flush with the cuvette cover while also leaving room for the intake tubing from the peristaltic pump  to fit through the gap 
on the bottom of the cuvette cover.Then run the tubing out from the peristaltic pump  It should look something like this:
...
### Connecting Wires
When trying to connect jumpers with longer wires you will likely want to use solder wrap. If you don't have it you will just have to cut wires and solder them together and then cover with either 
...
by running the following code in Thonny on a raspberry picco
...
Next we will connect out RGB light, first you should test to see what pin corresponds to what color for the light. Using a multi meter put the ground on the longest pin and the positive on any other
...
If this causes your light to flash red then blue the green over 2 second intervals then you successfully worked the pins. If the colors do not go in that order you may have wired things differently so
...
the COM side of the relay that you plugged GP19 into. For GP18 and GP20 relays they should have power coming from the 12V bus into the COM section. Plug The ground from the peristaltic pump and the
valve into the ground for the 12V bus. Plug the DV pump ground into the 3V bus ground. Now plug the DV Pump power into the NO section of the GP19 relay. Connect the peristaltic pump power into the 
GP18 Relay NO section and finally the valve power into the GP20 NO section. If done correctly you should be able to run this code
...
That is all of the wiring congratulations you are almost done!

## Calibrating the Machine
When calibrating your machine it is best to have the most samples that you can. You want water that is ranging from 5pH to 10pH to create a better curve. The more data you give the machine the more accurate
...
```
You will need to remove the 207.0545. If everything up to this point has been working then you should be able to run pc_side_collection.py. Disconnect your pi and reconnect it. It should flash a blue light first and 
then you will be able to run the file. Once it is finished you will get back a csv with data in it. That means everything is working properly. Fill your cuvette with clear water and repeat the same process starting 
...
Change the x to your known pH and the y to the corresponding value that you got. The data in there is from my calibration curve if you would like to use it you can but it will not be as accurate as your sensor will have 
...
Just remember that when you want to test you would just run pc_side_collection.py Here the water pump waits for an extra second after the dye pump turns off and continues pumping and then turns off. Continue messing with this until you 
